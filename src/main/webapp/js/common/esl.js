/**
 * ESL (Enterprise Standard Loader) Copyright 2013 Baidu Inc. All rights
 * reserved.
 * 
 * @file Browser端标准加载器，符合AMD规范
 * @author errorrik(errorrik@gmail.com) Firede(firede@firede.us)
 */
...
(function(global) {
	...
	function waitTimeoutNotice() {
		...
		for ( var id in modModules) {
			...
			each(modModules[id].realDeps || [], function(depId) {
	 ...
	 *            id 模块标识
	 * @param {Array.
	 *            <string>} dependencies 显式声明的依赖模块列表
	 * @param {*}
	 *            factory 模块定义函数或模块对象
	 */
	 ...
	 * 预分析模块
	 * 
	 * 首先，完成对factory中声明依赖的分析提取 然后，尝试加载"资源加载所需模块"
	 * 
	...
	function modPreAnalyse() {
		...
		each(modules, function(module) {
			...
			if (isFunction(factory)) {
				factory.toString().replace(commentRule, '').replace(
						requireRule, function($0, $1, $2) {
							realDepends.push($2);
						});
			}

			// 分析resource加载的plugin module id
			each(realDepends, function(dependId) {
	...
	/**
	 * 分析模块 对所有依赖id进行normalize化，完成分析，并尝试加载其依赖的模块
	 * 
	 * @inner
	 * @param {Array}
	 *            modules 模块对象列表
	...
	function modAnalyse(modules) {
		...
		each(
				...
				function(module) {
					...
					var hardDependsCount = isFunction(module.factory) ? module.factory.length
							: 0;

					each(depends, function(dependId, index) {
					...
					while (len--) {
						...
						if (!dependId || dependId in existsDepend
	...
	/**
	 * 等待模块依赖加载完成 加载完成后尝试调用factory完成模块定义
	 * 
	 * @inner
	 * @param {Object}
	 *            module 模块对象
	...
	function modWaitDependenciesLoaded(module) {
		...
		function checkInvokeReadyState() {
			...
			each(module.realDeps, function(depId) {
			...
			isReady && each(tryDeps, function(depId) {
		...
		function invokeFactory() {
			if (module.state == MODULE_STATE_DEFINED || checkingLevel > 1
			...
			try {
				var factory = module.factory;
				var exports = isFunction(factory) ? factory.apply(global,
						modGetModulesExports(module.deps, {
	...
	/**
	 * 根据模块id数组，获取其的exports数组 用于模块初始化的factory参数或require的callback参数生成
	 * 
	 * @inner
	 * @param {Array}
	 *            modules 模块id数组
	 * @param {Object}
	 *            buildinModules 内建模块对象
	 * @return {Array}
	 */
	function modGetModulesExports(modules, buildinModules) {
		var args = [];
		each(modules, function(moduleId, index) {
			args[index] = buildinModules[moduleId]
	...
	function modFireDefined(id) {
		modFireLevel++;
		each(modDefinedListener, function(listener) {
	...
	/**
	 * 清理模块定义完成事件监听器 modRemoveDefinedListener时只做标记 在modFireDefined执行清除动作
	 * 
	 * @inner
	 * @param {Function}
	 *            listener 模块定义监听器
	...
	function modSweepDefinedListener() {
		if (modFireLevel < 1) {
			modRemoveListenerIndex.sort(function(a, b) {
				return b - a;
			});

			each(modRemoveListenerIndex, function(index) {
	...
	function modRemoveDefinedListener(listener) {
		each(modDefinedListener, function(item, index) {
			if (listener == item) {
	...
	function modIsDefined(id) {
		return modExists(id) && modModules[id].state == MODULE_STATE_DEFINED;
	}
	...
	function modIsAnalyzed(id) {
		return modExists(id) && modModules[id].state >= MODULE_STATE_ANALYZED;
	}
	...
	function completePreDefine(currentId) {
		...
		each(preDefines, function(module) {
	...
	function nativeRequire(ids, callback, baseId) {
		...
		/**
		 * 尝试完成require，调用callback 在模块与其依赖模块都加载完时调用
		 * 
		 * @inner
		 */
	...
	function loadResource(pluginAndResource, baseId) {
		...
		function load(plugin) {
			if (!modIsDefined(pluginAndResource)) {
				plugin.load(resourceId, createLocalRequire(baseId),
						pluginOnload, moduleConfigGetter.call({
			...
			if (isString(requireId)) {
				...
				if (!(requiredModule = requiredCache[requireId])) {
					requiredModule = nativeRequire(
							normalize(requireId, baseId), callback, baseId);
	...
	function normalize(id, baseId) {
		...
		if (resourceId) {
			...
			resourceId = module && module.normalize ? module.normalize(
					resourceId, function(resId) {
	...
	/**
	 * 获取当前script标签 用于ie下define未指定module id时获取id
	 * 
	 * @inner
	 * @return {HTMLDocument}
	 */
	 ...
	 * @param {Array}
	 *            source 数组源
	 * @param {function(Array,Number):boolean}
	 *            iterator 遍历函数
	 */
